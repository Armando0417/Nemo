...
// -- Config -------------------------------------------------------
constexpr const char* JF_EXE    = R"(E:\Wandering_Sea\Sarcophagus\HTV_Tomb\engine\jellyfin.exe)";
constexpr const char* JF_DATA   = R"(E:\Wandering_Sea\Gates\HTV\data)";
constexpr const char* JF_CACHE  = R"(E:\Wandering_Sea\Gates\HTV\cache)";
constexpr const char* JF_LOGS   = R"(E:\Wandering_Sea\Gates\HTV\logs)";
constexpr const char* SERVE_DIR = R"(D:\Wandering_Sea)";
constexpr int         JF_PORT   = 3434;
...
// -- Types --------------------------------------------------------
enum class ServerStatus { NOT_RUNNING, STARTING_UP, READY, ERR };
...
ServerStatus checkHealth(int port)
{
    ...
    if (sock == INVALID_SOCKET) { WSACleanup(); return ServerStatus::ERR; }
    ...
    {
        closesocket(sock); WSACleanup(); return ServerStatus::ERR;
    }
    ...
    if (response.find("Healthy")  != std::string::npos) return ServerStatus::READY;
    if (response.find("Degraded") != std::string::npos) return ServerStatus::STARTING_UP;
    return ServerStatus::ERR;
}
...
bool launchSilent(const std::string& cmd, const char* workingDir = nullptr)
{
    ...
    return CreateProcessA(NULL, mutableCmd.data(), NULL, NULL, FALSE,
                          CREATE_NO_WINDOW, NULL, workingDir, &si, &pi);
}

bool launchJellyfin()
{
    std::string cmd = std::string("\"") + JF_EXE + "\""
        + " --datadir \""  + JF_DATA  + "\""
        + " --cachedir \"" + JF_CACHE + "\""
        + " --logdir \""   + JF_LOGS  + "\"";
