...
const distDir = path.join(nemoRoot, 'ishtar_frontend_dist');
const basePath = process.env.NEMO_ISHTAR_BASE_PATH ?? '/ishtar';
...
if (!response.ok) {
	throw new Error(`Failed to render Ishtar frontend shell: ${response.status} ${response.statusText}`);
}

fs.writeFileSync(path.join(distDir, 'index.html'), await response.text(), 'utf8');
console.log(`Exported Nemo Ishtar frontend to ${distDir}`);
