...
from data_schemas import Gallery, GalleryTagLink, Tag
from gallery_source import load_metadata, normalize_metadata
...
class Librarian:
    class _Library:
        ...
        def build_shelves(self):
            print("--- Librarian is preparing the Tomb (Creating Tables) ---")
            SQLModel.metadata.create_all(self.engine)

    def __init__(self, db_name: str = "tomb.db", debug: bool = True):
    ...
    def _ensure_query_support(self):
        """Provision indexes and title FTS support for filter-heavy routes."""
        statements = [
    ...
    def _get_or_create_tag(self, session: Session, name: str, category: str) -> Tag:
        """Finds or creates a tag to avoid duplicates in the many-to-many system."""
    ...
    def archive_gallery(self, gallery_folder: str):
        """
        Processes a folder of images, reads its metadata.json, 
        and sets the is_completed flag based on failed_pages.json presence.
        """
        ...
        with Session(self.engine) as session:
            ...
            new_gallery = Gallery(
                ...
                is_completed=normalized["is_completed"] # True if NO failed_pages.json exists
            )

            # Map the Metadata into the Tag system
            tags_to_link = []
            
            # Categories from your metadata template
            ...
            for t_name in normalized.get("tags", []):
                tags_to_link.append(self._get_or_create_tag(session, t_name, "tag"))

            new_gallery.tags = tags_to_link
            session.add(new_gallery)
            session.commit()
            session.refresh(new_gallery)
            return new_gallery
        
    def archive_gallery_from_disk(self, gallery_folder: str):
        """
        Parses a specific folder to archive its gallery and tags.
        """
        meta = load_metadata(gallery_folder)
        if not meta:
            return None
        ...
        with Session(self.engine) as session:
            # Prevent duplicates using media_id from metadata
            existing = session.exec(
                select(Gallery).where(Gallery.media_id == normalized["media_id"])
            ).first()
            if existing:
            ...
            new_gallery = Gallery(
                media_id=normalized["media_id"], # "3320580"
                title=normalized["title"],       # "1/5 no Renai Kanjou..."
                page_count=normalized["page_count"], # 126
                upload_date=normalized["upload_date"], # "2025-04-19"
                path=normalized["path"],
                is_completed=normalized["is_completed"]
            )

            # Process Many-to-Many Tags based on your template categories
            tags_to_link = self._build_tags(session, normalized)
    ...
    def _build_tags(self, session: Session, normalized: dict):
        tags_to_link = []
        tags_to_link.append(self._get_or_create_tag(session, normalized["artist"], "artist"))
        tags_to_link.append(self._get_or_create_tag(session, normalized["series"], "series"))
